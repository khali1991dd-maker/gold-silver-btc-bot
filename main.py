...
if not is_open(now):
    send(f"⏰ الوقت: {w_time} - فريم 5 دقائق\n🥇 السوق مغلق")
    exit()

# جلب البيانات - تم تصحيح الرمز
...
if price is None:
    price=float(c.iloc[-1]); src="احتياطي GC=F"
...
if abs(price-f50) <= distance and price > ema20 and r < 65:
    signal="شراء"; entry=f50; reason=f"قريب من فيبو 50% ({f50:.1f}) + فوق متوسط 20"
elif abs(price-f25) <= distance and price > ema50:
    signal="شراء"; entry=f25; reason=f"قريب من فيبو 25% ({f25:.1f})"
elif abs(price-f100) <= distance and price < ema20 and r > 35:
    signal="بيع"; entry=f100; reason=f"قريب من فيبو 100% ({f100:.1f}) + تحت متوسط 20"
elif abs(price-f0) <= distance and price < ema50:
    signal="بيع"; entry=f0; reason=f"قريب من فيبو 0% ({f0:.1f})"
...
if signal:
    ...
    msg=(f"⚡️ اشارة {signal} - فريم 5 دقائق\n"
           f"⏰ الوقت: {w_time}\n"
    ...
    send(f"⏰ الوقت: {w_time} - فريم 5 دقائق\n"
         ...
         f"🚫 لا توجد اشارة حالياً - السعر بعيد عن مناطق فيبو والطلب بأكثر من {distance} دولار (اقرب مسافة {nearest:.1f}$) - ننتظر رجوع السعر")
