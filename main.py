...
def send(t):
    ...
    try:
        requests.post(f"https://api.telegram.org/bot{TG_TOKEN}/sendMessage",
                      data={"chat_id": TG_CHAT, "text": t}, timeout=10)
    except: pass
    try:
        if WA_KEY:
            requests.get(f"https://api.callmebot.com/whatsapp.php?phone={WA_PHONE}&text={requests.utils.quote(t)}&apikey={WA_KEY}", timeout=10)
    except: pass
    print(t)
...
def calc(df):
    ...
    e12=df['Close'].ewm(12).mean()
    e26=df['Close'].ewm(26).mean()
    df['MACD']=e12-e26
...
for name,sym in syms.items():
    ...
    hi=df['High'].tail(40).max()
    lo=df['Low'].tail(40).min()
    fibo78=hi-(hi-lo)*0.78
    fibo61=hi-(hi-lo)*0.618
...
{ob}
الشمعة: {candle}
MA10:{ma[10]:.1f} MA20:{ma[20]:.1f} MA30:{ma[30]:.1f} MA50:{ma[50]:.1f} MA70:{ma[70]:.1f} MA100:{ma[100]:.1f} MA200:{ma[200]:.1f}""")
...
if NOW.minute<5:
    ...
    for name,sym in syms.items():
        if name!="بيتكوين" and gold_closed():
            rep+=f"🔒 {name}: مغلق جمعة 11م - اثنين 1ص\n"
            continue
...
if os.getenv("GITHUB_EVENT_NAME")=="workflow_dispatch":
    send(f"✅ البوت تفعل الان {NOW.strftime('%A %Y-%m-%d %I:%M:%S %p')} توقيت عمان\nرقمك 96897134774 مسجل - الذهب مغلق جمعة 11م - اثنين 1ص")
