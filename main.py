...
def send(t):
    requests.post(f"https://api.telegram.org/bot{TG_TOKEN}/sendMessage",
                  data={"chat_id": TG_CHAT, "text": t}, timeout=15)
    print(t)
...
for name,sym in syms.items():
    ...
    candle="دوجي" if is_doji else "شمعة شرائية قوية" if bull else "شمعة بيعية قوية" if bear else "شرائية" if l['Close']>l['Open'] else "بيعية"
    ...
    if signal and prev.get(name)!=signal:
        if prev.get(name): send(f"🔄 تغيير ترند {name}: {prev.get(name)} -> {signal}\n{NOW.strftime('%A %Y-%m-%d %I:%M:%S %p')}")
