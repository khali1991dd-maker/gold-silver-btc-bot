...
try:
    ...
    # تنبيه قبل الخبر بساعة 3:30م -> خبر 4:30م
    if NOW.hour==15 and NOW.minute<30:
        send(f"⚠️ *تنبيه خبر مهم*\nالساعة 4:30م بتوقيت مسقط خبر قوي على الدولار\n⏰ {NOW.strftime('%Y-%m-%d %H:%M')} مسقط")

    # للتجربة خليها True عشان يرسل الحين - بعدين رجعها لـ NOW.minute<5
    is_hourly = True
    ...
    for sym,name in [("GC=F","الذهب"),("SI=F","الفضة"),("BTC-USD","البيتكوين")]:
        ...
        if df is None or len(df)<200:
            send(f"⚠️ {name} لا بيانات {len(df) if df is not None else 0}\n⏰ {NOW.strftime('%H:%M')} مسقط")
            continue
        ...
        block="يوجد بلوك اوردر" if rng>atr*1.8 else "لا يوجد"

        price=float(c.iloc[-1])

        # ✅ بدون دخول عرضي: لازم 6 موفنجات مرتبة
        ...
        # ✅ ما يبيع اذا RSI تحت 25 وما يشتري فوق 75 + شمعة توافق الترند + مو دوجي
